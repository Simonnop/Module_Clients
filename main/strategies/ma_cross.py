...
MA_SLOW_DEFAULT = int(os.getenv('MA_SLOW_PERIOD', '60'))
...
def evaluate_volume_condition(stock_code: str, lookback: int = 5) -> Tuple[bool, Optional[float], Optional[float], Optional[str]]:
    """检查当前成交量是否达到过去几个交易日的平均成交量"""
    volumes = common.fetch_close_volume_history(stock_code, lookback)
    if len(volumes) < lookback:
        return False, None, None, f"过去{lookback}个交易日的成交量数据不足"

    average_volume = sum(volumes[-lookback:]) / len(volumes[-lookback:])
    current_volume = common.fetch_current_volume(stock_code)
    if current_volume is None:
        return False, average_volume, None, "当前成交量获取失败"

    if current_volume >= average_volume:
        return True, average_volume, current_volume, None

    return False, average_volume, current_volume, "当前成交量低于过去五个交易日的平均值"
...
def run_ma_cross_monitor(data: Dict[str, Any], items: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for entry in items:
        ...
        if fast_period != MA_FAST_DEFAULT or slow_period != MA_SLOW_DEFAULT:
            msg = (
                f"{stock_code} 当前双均线策略仅支持 fast={MA_FAST_DEFAULT} slow={MA_SLOW_DEFAULT} 组合，请调整配置"
            )
            logger.warning(msg)
            results.append({
                'code': stock_code,
        ...
        current_volume = None
        volume_message = None
        if cross_type == 'gold_cross':
            (
                volume_condition,
                volume_avg,
                current_volume,
                volume_message
            ) = evaluate_volume_condition(stock_code)
            if volume_condition:
                logger.info(
                    f"{display_name}({stock_code}) 量能条件满足，当前成交量{current_volume:.2f}，过去均量{volume_avg:.2f}"
                )
            else:
                logger.info(
                    f"{display_name}({stock_code}) 量能条件未通过: {volume_message}"
                )
        ...
        if cross_type == 'gold_cross' and volume_condition and notified_map.get(stock_code) != cross_type:
            ...
            body = (
                ...
                f"当前成交量: {current_volume:.2f}\n"
                f"过去5日均量: {volume_avg:.2f}\n"
        ...
        elif cross_type == 'gold_cross' and not volume_condition:
            alert_message = volume_message or "成交量未达到过去五日平均值"
